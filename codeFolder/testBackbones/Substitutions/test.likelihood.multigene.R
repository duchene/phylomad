...
if("testPlots" %in% unlist(input$whatToOutput)){
	...
	statlabels <- vector()
	if("chisq" %in% selectedStats) statlabels <- c(statlabels, "χ2 statistic")
	if("multlik" %in% selectedStats) statlabels <- c(statlabels, "Multinomial likelihood")
	if("delta" %in% selectedStats) statlabels <- c(statlabels, "δ statistic")
